...
import {
  ...
  Award, Target, Star, FlaskConical, Activity
} from "lucide-react";
import {
  teams, weeklyHighlights, xpActivities, MAX_XP_SEMESTER,
  courseInfo, currentWeek, type Team
...
// Helper: calculate team total XP
function getTeamXP(team: Team) {
  return team.members.reduce((sum, m) => sum + m.xp, 0);
}

// Helper: calculate team average XP
function getTeamAvg(team: Team) {
  return getTeamXP(team) / team.members.length;
}
      ...
      <div className="relative" style={{ width: size, height: size }}>
        <svg width={size} height={size} className="-rotate-90">
          <circle
            ...
            fill="none" stroke="oklch(0.245 0.03 264.052)" strokeWidth="8"
          />
          <motion.circle
            cx={size / 2} cy={size / 2} r={radius}
...
function TeamCard({ team, rank }: { team: Team; rank: number }) {
  ...
  const maxMemberXP = Math.max(...team.members.map(m => m.xp));
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="mt-1.5">
            <XPBar value={totalXP} max={MAX_XP_SEMESTER * team.members.length} color={team.color} />
          </div>
        </div>
        <div className="text-right shrink-0 ml-2">
          <div className="font-mono font-bold text-lg" style={{ color: team.color }}>
            {totalXP.toFixed(1)}
          </div>
                  ...
                  .map((member, idx) => (
                    <div key={member.name} className="flex items-center gap-3 py-1.5">
                      ...
                      <div className="w-24 sm:w-32">
                        <XPBar value={member.xp} max={MAX_XP_SEMESTER} color={team.color} />
                      </div>
                      <span className="font-mono text-sm font-medium w-12 text-right" style={{ color: team.color }}>
                        {member.xp.toFixed(1)}
                      </span>
                    </div>
                  ))}
...
export default function Home() {
  ...
  const rankedTeams = useMemo(() => {
    return [...teams].sort((a, b) => getTeamXP(b) - getTeamXP(a));
  }, []);

  // Get top 10 individual students
  const topStudents = useMemo(() => {
    const all = teams.flatMap(t =>
      t.members.map(m => ({ ...m, teamName: t.name, teamColor: t.color, teamEmoji: t.emoji }))
    );
    return all.sort((a, b) => b.xp - a.xp).slice(0, 10);
  }, []);

  // Stats
  const totalXPEarned = useMemo(() => teams.reduce((s, t) => s + getTeamXP(t), 0), []);
  const avgXPPerStudent = totalXPEarned / courseInfo.totalStudents;
  const topTeam = rankedTeams[0];
  const topTeamXP = getTeamXP(topTeam);

  // Latest highlight
  const latestHighlight = weeklyHighlights[weeklyHighlights.length - 1];
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="relative overflow-hidden">
        ...
        <div className="relative container pt-8 pb-12 sm:pt-12 sm:pb-16">
          <div className="flex items-center gap-3 mb-2">
            ...
            <span className="text-sm font-medium text-primary tracking-wide uppercase font-display">
              {courseInfo.university} — {courseInfo.name} — {courseInfo.semester}
            </span>
          </div>
          ...
          <div className="flex items-start gap-4 sm:gap-6">
            <div className="flex-1">
              <h1 className="font-display font-extrabold text-3xl sm:text-5xl text-foreground leading-tight">
                Leaderboard
                <span className="text-gradient-emerald"> XP</span>
              </h1>
              <p className="mt-2 text-muted-foreground text-sm sm:text-base max-w-xl font-body">
                Acompanhe a pontuação das equipes e dos alunos em tempo real.
                Semana {currentWeek} de 16 do semestre.
              </p>
            </div>
            <img
              src={TROPHY_ICON}
              alt="Troféu"
              className="w-16 h-16 sm:w-24 sm:h-24 object-contain shrink-0 drop-shadow-lg hidden sm:block"
            />
          </div>
          ...
          <div className="mt-8 grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4">
            {[
              { icon: <Users size={18} />, label: "Equipes", value: courseInfo.totalTeams.toString(), sub: `${courseInfo.totalStudents} alunos` },
              { icon: <Zap size={18} />, label: "XP Total Turma", value: totalXPEarned.toFixed(0), sub: `de ${MAX_XP_SEMESTER * courseInfo.totalStudents}` },
              ...
              <motion.div
                ...
                style={{ backgroundColor: "oklch(0.195 0.03 264.052 / 0.8)" }}
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
      ...
      {latestHighlight && (
        <div className="container -mt-2 mb-6">
          <motion.div
            className="border border-primary/20 rounded-lg p-4 flex items-start gap-3 glow-emerald"
              ...
              <div className="grid grid-cols-3 gap-3 sm:gap-4 mb-8">
                {[rankedTeams[1], rankedTeams[0], rankedTeams[2]].map((team, idx) => {
                  ...
                  return (
                    <motion.div
                      key={team.id}
                      className={`border rounded-lg p-3 sm:p-5 text-center ${
                        isFirst ? "border-amber-500/30 glow-amber" : "border-border"
                      }`}
                      style={{
                        backgroundColor: isFirst ? "oklch(0.795 0.16 86.047 / 0.06)" : "oklch(0.195 0.03 264.052)",
                    ...
                    >
                      ...
                      <div className={`text-xs font-mono font-bold mb-1 ${
                        actualRank === 1 ? "text-amber-400" : actualRank === 2 ? "text-gray-400" : "text-amber-700"
                      }`}>
                        #{actualRank}
                      </div>
                      <div className="font-display font-bold text-sm sm:text-base text-foreground truncate">{team.name}</div>
                      <div className="font-mono font-bold text-xl sm:text-2xl mt-1" style={{ color: team.color }}>
                        {getTeamXP(team).toFixed(1)}
                      </div>
                      <div className="text-[11px] text-muted-foreground">XP total</div>
                    </motion.div>
                  );
                ...
                {topStudents.slice(0, 3).map((student, idx) => (
                  ...
                  >
                    ...
                    <div className="mt-2 text-center">
                      <div className="font-display font-semibold text-sm text-foreground truncate max-w-[100px]">
                        {student.name}
                      </div>
                      <div className="text-[11px] text-muted-foreground flex items-center gap-1 justify-center">
                        <span>{student.teamEmoji}</span>
                ...
                {topStudents.map((student, idx) => (
                  <motion.div
                    ...
                    className={`flex items-center gap-3 sm:gap-4 p-3 sm:p-4 ${
                      idx < topStudents.length - 1 ? "border-b border-border/50" : ""
                    }`}
                  ...
                  >
                    ...
                    <div
                      ...
                      style={{ backgroundColor: student.teamColor + "22", border: `1px solid ${student.teamColor}44` }}
                    >
                      {student.teamEmoji}
                    </div>
                    <div className="flex-1 min-w-0">
                    ...
                    <div className="w-20 sm:w-32">
                      <XPBar value={student.xp} max={MAX_XP_SEMESTER} color={student.teamColor} />
                    </div>
                    <div className="font-mono font-bold text-sm w-12 text-right" style={{ color: student.teamColor }}>
                      {student.xp.toFixed(1)}
                    </div>
                  </motion.div>
                ))}
              </div>
            ...
            >
              ...
              <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 mb-10">
                {xpActivities.map((act, idx) => (
                  ...
                  >
                    ...
                    <div className="flex-1">
                      <div className="font-display font-semibold text-sm text-foreground">{act.name}</div>
                      <div className="text-xs text-muted-foreground mt-0.5">Até {act.maxXP} XP por sessão</div>
                    </div>
              ...
              <div className="relative">
                ...
                <div className="grid gap-4">
                  {weeklyHighlights.map((highlight, idx) => (
                    <motion.div
                      key={highlight.week}
      ...
      <footer className="border-t border-border py-6">
        <div className="container text-center">
          <p className="text-xs text-muted-foreground">
            {courseInfo.university} — {courseInfo.name} ({courseInfo.code}) — Prof. Dr. — Semestre {courseInfo.semester}
          </p>
          <p className="text-[11px] text-muted-foreground/60 mt-1">
            Atualizado em Semana {currentWeek} • Sistema de Gamificação com Metodologias Ativas
          </p>
        </div>
      </footer>
    </div>
