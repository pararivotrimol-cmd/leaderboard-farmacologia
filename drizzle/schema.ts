...
/**
 * Online Meetings - monitors schedule online sessions for student Q&A
 * Monitors create meetings with a link (Google Meet, Zoom, etc.)
 * Students can see upcoming meetings and join via the link
 */
export const onlineMeetings = mysqlTable("onlineMeetings", {
  id: int("id").autoincrement().primaryKey(),
  // Meeting title/topic
  title: varchar("title", { length: 300 }).notNull(),
  // Description of what will be covered
  description: text("description"),
  // Name of the monitor hosting the meeting
  ...
  // Platform type
  platform: mysqlEnum("platform", ["google_meet", "zoom", "teams", "discord", "other"]).default("google_meet").notNull(),
  // Scheduled date and time (stored as UTC timestamp)
  scheduledAt: timestamp("scheduledAt").notNull(),
  // Duration in minutes
  durationMinutes: int("durationMinutes").notNull().default(60),
  // Module/topic related to the meeting
  module: varchar("module", { length: 100 }).notNull().default("Geral"),
  ...
  recurrence: mysqlEnum("recurrence", ["none", "weekly"]).default("none").notNull(),
  createdAt: timestamp("createdAt").defaultNow().notNull(),
  updatedAt: timestamp("updatedAt").defaultNow().onUpdateNow().notNull(),
});

export type OnlineMeeting = typeof onlineMeetings.$inferSelect;
export type InsertOnlineMeeting = typeof onlineMeetings.$inferInsert;
